...
public strictfp class LauncherRobot extends Robot {
    ...
    public static final int GIVE_UP_TURNS = 10;
    ...
    public void run() throws GameActionException {
        ...
        if (target != null) {
            int targetDist = rc.getLocation().distanceSquaredTo(target);
            if (targetDist > closestApproach) {
                closestApproach = targetDist;
    ...
    public boolean tryMove() throws GameActionException {
        ...
        if (target != null && targetWeight == REPORTED_BASE_WEIGHT) {
            if (nearestReportedEnemy != null && target.distanceSquaredTo(nearestReportedEnemy) > 4) {
                ...
                closestApproach = curr.distanceSquaredTo(target);
                turnsSinceProgress = 0;
            }
        }

        if (target != null && turnsSinceProgress > GIVE_UP_TURNS) {
    ...
    public MapLocation getTarget(RobotController rc) throws GameActionException {
        ...
        if (!rc.isMovementReady()) {
            if (nearestReportedEnemy != null
                    && nearestReportedEnemy.isWithinDistanceSquared(curr, RobotType.LAUNCHER.actionRadiusSquared)
                    && !rc.canSenseLocation(nearestReportedEnemy)) {
                return nearestReportedEnemy;
            }

            /*
             * for (MapLocation l : rc.getAllLocationsWithinRadiusSquared(curr,
             * RobotType.LAUNCHER.actionRadiusSquared)) { if (!rc.canSenseLocation(l)) {
             * System.out.println("GOT RANDO TARGET"); rc.setIndicatorDot(l, 0, 255, 0);
             * return l; } else { rc.setIndicatorDot(l, 200, 0, 0); } }
